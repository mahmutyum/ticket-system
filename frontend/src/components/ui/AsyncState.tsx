    ...
    <div className="animate-pulse divide-y divide-gray-100 dark:divide-slate-800" aria-label={t('common.loading')}>
      {Array.from({ length: rows }, (_, index) => (
        <div key={index} className="flex items-center gap-4 px-5 py-4">
          <div className="h-9 w-9 rounded-inset surface-2" />
...
export function EmptyState({ title, description, icon: Icon = Inbox }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center px-6 py-14 text-center">
      <span className="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-inset bg-primary-50 text-primary-600 dark:bg-primary-500/10 dark:text-primary-300"><Icon className="h-6 w-6" /></span>
...
export function ErrorState({ title, description, retryLabel, onRetry }: ErrorStateProps) {
  return (
    <div className="flex flex-col items-center justify-center px-6 py-10 text-center" role="alert">
      <span className="mb-4 inline-flex h-12 w-12 items-center justify-center rounded-inset bg-red-50 text-red-600 dark:bg-red-500/10 dark:text-red-300">
        <AlertTriangle className="h-6 w-6" />
      </span>
      <h3 className="section-title">{title}</h3>
      {description && <p className="mt-1 max-w-sm text-sm text-muted">{description}</p>}
      {onRetry && retryLabel && <button type="button" onClick={onRetry} className="btn-secondary mt-4 text-sm">{retryLabel}</button>}
    </div>
  );
}
